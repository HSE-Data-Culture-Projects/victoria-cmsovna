async function addTask() {
    ...
    const topicIds = Array.from(document.getElementById('topic-ids').selectedOptions).map(opt => opt.value);
    ...
    try {
        ...
        const response = await fetch('/api/tasks', { method: 'POST', headers: { 'Authorization': `Bearer ${token}` }, body: formData });
...
function createTaskElement(task) {
    ...
    try {
        ...
        } else li.textContent = task.content;
    } catch {
...
async function updateTask(id) {
    ...
    try {
        const token = localStorage.getItem('token');
        const response = await fetch(`/api/tasks/${id}`, { method: 'PATCH', headers: { 'Authorization': `Bearer ${token}` }, body: formData });
        if (response.ok) loadTasks();
    } catch (err) {
...
function showTaskForm(taskId = '', taskContent = '', topicIds = []) {
    ...
    Array.from(topicSelect.options).forEach(opt => opt.selected = topicIds.includes(opt.value));
...
document.addEventListener('DOMContentLoaded', () => { loadTopicsForImport(); loadTasks(); });
